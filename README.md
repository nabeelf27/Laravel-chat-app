...
```
composer require nabeelfarooqi/chat
```
...
This will publish database migrations and a configuration file `nabeelfarooqi_chat.php` in the Laravel config folder.

## Configuration

See `nabeelfarooqi_chat.php` for configuration
...
Add the `nabeelfarooqi\Chat\Traits\Messageable` trait to any Model you want to participate in Conversations
For example, let's say we want out `Bot` model to chat with other Models:

```php

use Illuminate\Database\Eloquent\Model;
use nabeelfarooqi\Chat\Traits\Messageable;
...
Listen for `\nabeelfarooqi\Chat\Eventing\AllParticipantsDeletedMessage` and 

`\nabeelfarooqi\Chat\Eventing\AllParticipantsClearedConversation`
...
file `nabeelfarooqi_chat.php` as follows:
